...
/**
 * The company voucher portal content: buy a seat package from the wallet,
 * list purchased batches, and drill into a batch's team analytics + codes.
 * Rendered both on the branded dashboard's lower section and the standalone
 * company page. Assumes the caller has already gated on auth.
 */
export function CompanyVouchers({ assessmentId }: { assessmentId: string }) {
  ...
  return (
    <>
      ...
      <div className="lato-card">
        ...
        {packages.length === 0 ? (
          <p className="lato-note" style={{ marginTop: 16 }}>
